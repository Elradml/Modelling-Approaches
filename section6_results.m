...
exp = readmatrix('section6_data.csv');
x = exp(:, 1);
y = exp(:, 3);

%% k-w sst data
sst_lo = load('lo6Cpsst.dat');
sst_up = load('up6Cpsst.dat');
...
new_min = 0;
new_max = 0.47055; % local chord length based on section
sst_new_lo = ((sst_lo_sorted - lo_min_old) / (lo_max_old - lo_min_old) * (new_max - new_min) + new_min) / 0.47055; % divide by the max chord to normalize
% above airfoil
up_min_old = min(sst_up_sort);
up_max_old = max(sst_up_sort);
sst_new_up = ((sst_up_sort - up_min_old) / (up_max_old - up_min_old) * (new_max - new_min) + new_min) / 0.47055;

%% k-e rng data
rng_lo = load('lo6Cprng.dat');
rng_up = load('up6Cprng.dat');
xrng_lo = rng_lo(:, 1);
cplo_rng = rng_lo(:, 2);
xrng_up = rng_up(:, 1);
cpup_rng = rng_up(:, 2);
...
rng_new_lo = ((rng_lo_sort - lo_min_oldrng) / (lo_max_oldrng - lo_min_oldrng) * (new_max - new_min) + new_min) / 0.47055; % divide by the max chord to normalize
% above airfoil
up_min_oldrng = min(rng_up_sort);
up_max_oldrng = max(rng_up_sort);
rng_new_up = ((rng_up_sort - up_min_oldrng) / (up_max_oldrng - up_min_oldrng) * (new_max - new_min) + new_min) / 0.47055;
%% Spalart-Allmaras data
sp_lo = load('lo6Cpsp.dat');
sp_up = load('up6Cpsp.dat');
xsp_lo = sp_lo(:, 1);
cplo_sp = sp_lo(:, 2);
xsp_up = sp_up(:, 1);
cpup_sp = sp_up(:, 2);
...
sp_new_lo = ((sp_lo_sort - lo_min_oldsp) / (lo_max_oldsp - lo_min_oldsp) * (new_max - new_min) + new_min) / 0.47055; % divide by the max chord to normalize
% above airfoil
up_min_oldsp = min(sp_up_sort);
up_max_oldsp = max(sp_up_sort);
sp_new_up = ((sp_up_sort - up_min_oldsp) / (up_max_oldsp - up_min_oldsp) * (new_max - new_min) + new_min) / 0.47055;
%% ke standard data 
ke_lo = load('lo6Cpke.dat');
ke_up = load('up6Cpke.dat');
xke_lo = ke_lo(:, 1);
cplo_ke = ke_lo(:, 2);
xke_up = ke_up(:, 1);
cpup_ke = ke_up(:, 2);
...
ke_new_lo = ((ke_lo_sort - lo_min_oldke) / (lo_max_oldke - lo_min_oldke) * (new_max - new_min) + new_min) / 0.47055; % divide by the max chord to normalize
% above airfoil
up_min_oldke = min(ke_up_sort);
up_max_oldke = max(ke_up_sort);
ke_new_up = ((ke_up_sort - up_min_oldke) / (up_max_oldke - up_min_oldke) * (new_max - new_min) + new_min) / 0.47055;
%% kw standard data
kw_lo = load('lo6Cpkw.dat');
kw_up = load('up6Cpkw.dat');
xkw_lo = kw_lo(:, 1);
cplo_kw = kw_lo(:, 2);
xkw_up = kw_up(:, 1);
cpup_kw = kw_up(:, 2);
...
kw_new_lo = ((kw_lo_sort - lo_min_oldkw) / (lo_max_oldkw - lo_min_oldkw) * (new_max - new_min) + new_min) / 0.47055; % divide by the max chord to normalize
% above airfoil
up_min_oldkw = min(kw_up_sort);
up_max_oldkw = max(kw_up_sort);
kw_new_up = ((kw_up_sort - up_min_oldkw) / (up_max_oldkw - up_min_oldkw) * (new_max - new_min) + new_min) / 0.47055;
...
ylabel('C_p')
grid on
saveas(fig2, 'sec6_comparison.png')